...
def median_sigmaG(a, axis=None, overwrite_input=False, keepdims=False):
    ...
    """
    q25, median, q75 = np.nanpercentile(
        a, [25, 50, 75], axis=axis, overwrite_input=overwrite_input
    )
...
def fit_bivariate_normal(x, y, robust=False):
    ...
    if robust:
        ...
        r_xy = (sigmaG_u ** 2 - sigmaG_v ** 2) / (sigmaG_u ** 2 + sigmaG_v ** 2)
    ...
    sigma1 = np.sqrt(
        (
            0.5 * (sigma_x ** 2 + sigma_y ** 2)
            + np.sqrt(0.25 * (sigma_x ** 2 - sigma_y ** 2) ** 2 + sigma_xy ** 2)
        )
    )
    sigma2 = np.sqrt(
        (
